...
export function projectNztmPolygonToWgs84(polygon: Polygon): Polygon {
  ...
  return {
    type: "Polygon",
    coordinates: polygon.coordinates.map((ring) =>
      ring.map((position) => projectNztmPositionToWgs84(position)),
    ),
  };
}

export function projectNztmPositionToWgs84([east, north]: Position): [
  number,
  number,
] {
  const [longitude, latitude] = proj4(NZTM2000, WGS84, [east, north]);
  return [longitude, latitude];
}

export function projectWgs84PositionToNztm([longitude, latitude]: Position): [
  number,
  number,
] {
