...
export function renderPreliminaryReportHtml(
  ...
  const mapLegendHtml = `<div class="map-key" aria-label="Map key">${mapLegend.entries
    .map(
      (entry) =>
        `<span class="map-key-item"><i class="map-key-swatch ${esc(entry.kind)}${entry.dashed ? " dashed" : ""}" style="--map-colour:${esc(entry.colour)}"></i>${esc(entry.label)}</span>`,
    )
    .join("")}</div>`;
  ...
  *{box-sizing:border-box}html,body{margin:0;background:#eef2f4;color:#172126;font-family:Arial,sans-serif}body{-webkit-print-color-adjust:exact;print-color-adjust:exact}
  ...
  <section class="page">${header("Executive summary", 1)}
    ...
    <h2>Main aerial map</h2><img class="map summary-map" src="${report.mapImageDataUrl}" alt="Saved property and pool map">${mapLegendHtml}${mapAttributionHtml}
    ${footer}
  </section>
  <section class="page">${header("Property constraints", 2)}
    ...
    <img class="map" src="${report.mapImageDataUrl}" alt="Saved property and pool map">${mapLegendHtml}${mapAttributionHtml}${excludedMapLayers}
