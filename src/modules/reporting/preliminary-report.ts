...
export function renderPreliminaryReportHtml(
  ...
  const header = (section: string, page: number) =>
    `<header><div><b>Preliminary Feasibility Report</b><span>${esc(section)}</span></div><div>${esc(report.reference)}<br>Page ${page} of 3</div></header>`;
  ...
  const sources =
    report.sources.length > 0
      ? report.sources
          ...
          .map(
            (source) =>
              `<div><b>${esc(source.dataset)}</b> · ${esc(source.provider)} · ${esc(humanizeReportValue(source.status))}<br>${source.attribution ? `${esc(source.attribution)} · ` : ""}${esc(source.licence)}${source.sourceUrl ? ` · <a href="${esc(source.sourceUrl)}">${esc(source.sourceUrl)}</a>` : ""}</div>`,
          )
          .join("")
  ...
  const mapAttribution =
    report.sources.length > 0
      ? report.sources
          ...
          .map(
            (source) =>
              `<div>${source.attribution ? `${esc(source.attribution)} · ` : ""}${esc(source.licence)}${source.sourceUrl ? ` · <a href="${esc(source.sourceUrl)}">${esc(source.sourceUrl)}</a>` : ""}</div>`,
          )
          .join("")
  ...
  *{box-sizing:border-box}html,body{margin:0;background:#eef2f4;color:#172126;font-family:Arial,sans-serif}body{-webkit-print-color-adjust:exact;print-color-adjust:exact}
  ...
  <section class="page">${header("Executive summary", 1)}
    ...
    <h2>Main aerial map</h2><img class="map summary-map" src="${report.mapImageDataUrl}" alt="Saved property and pool map">
    ${footer}
  </section>
  <section class="page">${header("Property constraints", 2)}
    ...
    <img class="map" src="${report.mapImageDataUrl}" alt="Saved property and pool map">
    ...
    <div class="source-note"><b>Map attribution</b>${mapAttribution}</div>
    ${footer}
  </section>
